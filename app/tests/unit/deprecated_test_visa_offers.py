...
from app.tasks import add_card as t_add_card
from app.tasks import reactivate_card as t_reactivate_card
from app.tasks import remove_card as t_remove_card
...
def mock_celery_handler(action_code, card_info):
    {
        ...
        ActionCode.REACTIVATE: lambda: t_reactivate_card(card_info),
    }[action_code]()
...
class TestVisaOffers(TestCase):
            ...
            json.dumps(
                {
                    "responseStatus": {
                        "code": code,
                        "message": "un-enrol retry failure message",
                        "responseStatusDetails": [],
                    }
                }
            ),
            ...
            json.dumps(
                {
                    "responseStatus": {
                        "code": code,
                        "message": "un-enrol equivalent to success",
                        "responseStatusDetails": [],
                    }
                }
            ),
    ...
    def fail_response(response_headers, code="1000"):
        return [
            ...
            response_headers,
            json.dumps(
                {"responseStatus": {"code": code, "message": "un-enrol failure message", "responseStatusDetails": []}}
            ),
        ]
    ...
    def un_enrol_remove_card_scenario(self, scenario):
        httpretty.register_uri(httpretty.POST, self.vop_un_enrol_url, body=scenario)
    ...
    def un_enrol_scenario(self, scenario):
        ...
        resp = self.client.delete(
            self.metis_payment_card_endpoint,
    ...
    def activate_scenario(self, scenario):
        httpretty.register_uri(httpretty.POST, self.vop_activation_url, body=scenario)
        card_info = self.card_info_add[0]
    ...
    def deactivate_scenario(self, scenario):
        httpretty.register_uri(httpretty.POST, self.vop_deactivation_url, body=scenario)
        card_info = self.card_info_add[0]
        resp = self.client.post(
    ...
    def test_remove_card_retry_fail_on_2(self, mock_post):
        ...
        self.assertGreaterEqual(
            self.visa.MAX_RETRIES,
            3,
            f"Retries is set to {self.visa.MAX_RETRIES};" f" Must be >2 for this test to be valid ",
        )
    ...
    def test_remove_card_retry_success_on_3(self, mock_post):
        ...
        self.assertGreaterEqual(
            self.visa.MAX_RETRIES,
            3,
            f"Retries is set to {self.visa.MAX_RETRIES};" f" Must be >= 3 for this test to be valid ",
        )
