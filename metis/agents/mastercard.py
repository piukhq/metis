...
class MasterCard:
    ...
    def response_handler(self, response, action, status_mapping):
        if response.status_code >= 300:
            logger.warning(f"Mastercard {action} response: {response}, body: {response.text}")
            try:
            ...
            message = f"Problem connecting to PSP. Action: MasterCard {action}. Error:{psp_message}"
        ...
        if fault:
            ...
            logger.info(message)
            resp = {"message": f"{action} MasterCard Fault recorded. Code: {fault_code}", "status_code": 422}
        else:
