...
from metis.hermes import put_account_status
...
class Visa(AgentBase):
    ...
    def response_handler(self, response):
        ...
        if visa_data["state"] in ("pending", "succeeded"):
            ...
            message = "Visa {} successful - Token: {}, {}".format("batch", visa_data["token"], "Check Handback file")
        else:
    ...
    def request_body(self, card_info):
        ...
        body_data = (
            "{{#gpg}}" + self.visa_pem() + "," + recipient_id + "," + self.create_file_data(card_info) + "{{/gpg}}"
        )
        file_name = "LOYANG_REG_PAN_{}{}".format(str(int(time.time())), ".gpg")
        file_url = f"sftp://sftp.bink.com/{file_name}"

        def get_visa_action_code(action_code):
            return {ActionCode.ADD: "A", ActionCode.DELETE: "D"}[action_code]
    ...
    def create_cards(self, card_info):
        ...
        logger.info(f"POST URL {url}, header: {self.header}")
    ...
    def create_file_data(self, card_info):
        ...
        footer = Footer(record_count=str(detail_record_count).rjust(10, "0"), filler="")
...
-----END PGP PUBLIC KEY BLOCK-----""".replace(
    ...
    def get_next_seq_number(self):
        ...
        with psycopg2.connect(  # noqa: SIM117
            settings.POSTGRES_URI, connect_args=settings.POSTGRES_CONNECT_ARGS
        ) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT next_seq_number, sequence_date "
...
class VisaCardFile:
    ...
    def _serialize(field_class, fields):
        ...
        for item in field_class.fields:
            if item[2] == "F":
                ...
                start_str = "{}%-{}.{}s,".format("{{#format_text}}", item[1], item[1])
                end_str = "{{/format_text}}"
    ...
    def set_header(self, header):
        ...
        end_str = "{{/format_text}}"
        self.header_string = f"{start_str}{serialized_str}{end_str}"
    ...
    def set_footer(self, footer):
        ...
        start_str = "{{#format_text}}%-1000.1000s,"
        end_str = "{{/format_text}}\\n"
        self.footer_string = f"{start_str}{serialized_str}{end_str}"
